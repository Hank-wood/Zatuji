...
public class ZoomImageView extends ImageView {
        ...
        public boolean onTouch(View v, MotionEvent event) {
            ...
            switch (event.getAction() & MotionEvent.ACTION_MASK) {// 单点监听和多点触碰监听
                ...
                case MotionEvent.ACTION_MOVE:
                    // 拖拉图片
                    if(event.getX() - startPoint.x>=ViewConfiguration.get(getContext()).getScaledTouchSlop()
                            ||event.getY() - startPoint.y>= ViewConfiguration.get(getContext()).getScaledTouchSlop())
                        isClick = false;
                    if (mode == MODE_DRAG) {
    ...
    private void setDoubleTouchEvent(MotionEvent event) {
        ...
        if (currentTime - firstTouchTime >= intervalTime) {
            ...
            LogUtils.d("isClick:"+isClick);
            if(isClick && firstTouchTime == 0) {
                performClick();
                firstTouchTime = 0;
            }
        } else {
            ...
            if (Math.abs(event.getX() - firstPointF.x) < 30f
                ...
                // 判断当前缩放比例与最大最小的比例
                LogUtils.d("onDoubleTab");
                ZoomAnimate animate = new ZoomAnimate();
                if (values[Matrix.MSCALE_X] < mMaxScale) {
                    matrix.postScale(mMaxScale/ values[Matrix.MSCALE_X],
                            ...
                            event.getY());
                    animate.start(values[Matrix.MSCALE_X]
                            ,mMaxScale
                        ...
                        matrix.postScale(mMinScale/ values[Matrix.MSCALE_X],
                                ...
                                event.getY());
//                        setImageMatrix(matrix);
                        animate.start(values[Matrix.MSCALE_X],mMinScale,values[Matrix.MSCALE_Y], mMinScale,event.getX()
                                ,event.getY());
                }
            }
    ...
    private void zoomImageWithAnim(float scale,float current,float x,float y){
        matrix.postScale(scale/ current,
                scale/ current, x,
                y);
        setImageMatrix(matrix);
    }
    ...
    class ZoomAnimate{
        ...
        public void start(float fromX,float toX,float fromY,float toY,float pivotX,float pivotY){
            this.animation = new ScaleAnimation(fromX,toX,fromY,toY, Animation.RELATIVE_TO_SELF,pivotX,Animation.RELATIVE_TO_SELF,pivotY);
            animation.setDuration(duration);
            ZoomImageView.this.setAnimation(animation);
            animation.start();
        }
    }
}
